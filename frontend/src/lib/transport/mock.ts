    ...
    switch (command.type) {
      ...
      case 'mode_switch_confirm':
        if (command.accepted) {
          ...
          this.emit({
            ...
            text: 'Bạn chọn sửa kế hoạch. Vẫn đang ở Plan mode, chưa có giấy phép nào được cấp.',
          })
        }
        break
      ...
      case 'revoke_lease':
        this.emit({
          ...
          status: 'bi_thu_hoi',
          reason: 'Người dùng bấm Thu hồi ở bảng Nhãn & Giấy phép.',
        })
        break
      ...
      case 'interrupt':
        this.emit({
          ...
          text: `Đã nhận lệnh ngắt mức "${command.level}".`,
        })
        break
